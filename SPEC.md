...
> 본 문서 기준일: 2026-06-18
>
> ⚠️ **이 문서는 5단계 시절 기준.** 이후 6단계 재구성(설정→업로드→계획검수→AI생성→편집→발행), 계획 검수(plan), 문체 프로파일(B), 네이버 SEO 강화(E), 지역소개 강화(F1), 피드백 재작성(F2)이 추가됐다.
> **현재 단계별 흐름·기능·입력값은 [`docs/WORKFLOW.md`](docs/WORKFLOW.md)가 최신 문서.** 개별 기능 설계는 `docs/superpowers/specs/` 참고.
...
┌──────────────────────────────────────────────────────────────────┐
│                         Browser (index.html)                       │
│   5-Step SPA UI — 업로드 → 분석 → 계획 검수/SEO → 생성 → 발행        │
└───────────────────────────────┬──────────────────────────────────┘
                                 │ REST + 폴링(/api/status)
┌───────────────────────────────▼──────────────────────────────────┐
│                          app.py (Flask)                            │
