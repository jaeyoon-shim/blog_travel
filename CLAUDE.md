...
2. **본문 텍스트에 구글맵 URL을 넣지 않는다.** 네이버 SE3 에디터가 텍스트 중 URL을 감지하면 무조건 OG 링크카드로 변환해 레이아웃이 깨진다. 본문에는 `📍 장소명`, `📍 A → B 경로` 같은 **텍스트만** 표시한다. 단, **장소 지도 카드는 예외적으로 발행기가 독립 블록으로 URL을 붙여 OG 카드 변환을 의도적으로 유도**한다(`<!--MAPCARD:url|display-->` 마커 → `map_card` 블록 → `_paste_map_card`, 2026-07-03 I라운드). AI 프롬프트의 "URL 절대 금지" 규칙은 그대로 유지 — 카드 삽입은 항상 코드 몫이다.
